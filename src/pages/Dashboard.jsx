...
const Dashboard = () => {
  ...
  const [stats, setStats] = useState([
    { title: 'Active Resumes', value: 0, icon: <FileText size={24} /> },
    { title: 'Applications', value: 0, icon: <Briefcase size={24} /> },
    { title: 'Last Updated', value: 'Never', icon: <Clock size={24} /> },
  ]);
...
useEffect(() => {
  ...
  try {
    const response = await ApiService.get('/dashboard/resumes');
    setResumes(response.resumes || []);
    setStats([
      { title: 'Active Resumes', value: response.stats?.activeResumes || 0, icon: <FileText size={24} /> },
      { title: 'Applications', value: response.stats?.totalApplications || 0, icon: <Briefcase size={24} /> },
      { title: 'Last Updated', value: response.stats?.lastUpdated || 'Never', icon: <Clock size={24} /> },
    ]);
  } catch (error) {
...
useEffect(() => {
  if (!loading && (userData.fullName || userData.email)) {
    fetchDashboardData();
  }
}, [loading, userData]);
